...
| FOS-001 | Pin supported Node 22 runtime | FOS-000 | REQ-018 / AC-018 / TC-018 | LOCAL, CI | package/runtime docs | REVIEW | IMPLEMENTED | FULLY_VERIFIED | #1 | #2 |
...
- FOS-001: Dependency FOS-000 is DONE. GitHub Issue #1 and PR #2 are canonical. LOCAL passed on Node v22.23.2: clean `npm ci`, 111 test files and 963 tests, `npm run typecheck`, and `npm run build` with 24 static pages. Node 26 install was rejected with `EBADENGINE`. CI run 34826328734 passed install, tests, typecheck, and build on commit `df90829`. Implementation and required verification are complete; task remains REVIEW until PR review and merge disposition.
